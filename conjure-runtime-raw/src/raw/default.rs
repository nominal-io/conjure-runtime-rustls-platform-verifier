...
pub const TCP_KEEPALIVE: Duration = Duration::from_secs(3 * 60);
// Most servers time out idle connections after 60 seconds, so we'll set the client timeout a bit below that.
pub const HTTP_KEEPALIVE: Duration = Duration::from_secs(55);
...
pub fn load_certs_file(path: &Path) -> Result<Vec<CertificateDer<'static>>, Error> {
...
pub struct DefaultRawClient(pub Client<ConjureConnector, RawBody>);
